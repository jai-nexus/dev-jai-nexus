...
control-plane capability families at an exact repository and source/provenance
cutoff.
...
## 2. Control coordinates and source/provenance cutoff
...
| Source/provenance cutoff | `2026-07-22T23:14:26Z` |
| Completed local validation | `2026-07-22T23:32:18Z` |
...
The source/provenance cutoff bounds source and upstream evidence collection
only. `A9 cutoff` in the registries below means this source/provenance cutoff.
The completed local-validation timestamp records execution of the required
fifteen tests after that cutoff and does not expand it.
...
Within this inventory, `LOCAL_SHADOW` means deterministic app-local or
in-process evaluation, generation, rendering, or inspection under the row's
exact `runtime_mode`, without requiring durable state or external effects. It
does not mean a live shadow service; the D2-D8 founder operating loop unless
the row specifically identifies `A9-CAP-005`; deployed runtime; persistence;
provider dispatch; customer visibility; or execution or activation authority.
...
| `A9-CAP-002` | Governed route-contract, adoption, boundary, parity, and decision seam | Bounded passalong, motion-intake, and manual-inference response contracts, route decisions, route exports, and synthetic test-harness boundaries. | [CAP002-PASSALONG-CONTRACT]<br>[CAP002-MOTION-CONTRACT]<br>[CAP002-INFERENCE-CONTRACT]<br>[CAP002-PASSALONG-DECISION]<br>[CAP002-MOTION-DECISION]<br>[CAP002-INFERENCE-DECISION]<br>[CAP002-HARNESS-FIXTURES]<br>[CAP002-HARNESS-PROVIDER-SEAM]<br>[CAP002-HARNESS-PERSISTENCE-SEAM]<br>[CAP002-HARNESS-ROUTE-RESPONSE]<br>[CAP002-PASSALONG-COLLECTION-ROUTE]<br>[CAP002-PASSALONG-DETAIL-ROUTE]<br>[CAP002-MOTION-ROUTE]<br>[CAP002-INFERENCE-ROUTE] | Collection `GET`/`POST`; detail `GET`/`PATCH`; motion-intake `GET`/`POST`; manual-inference `POST`; `decidePassalongCollectionList`; `decidePassalongCollectionCreate`; `decidePassalongDetailPatch`; `decideMotionIntakeList`; `decideMotionIntakeCreate`; `decideManualInferenceRun`; synthetic fixture, provider-seam, persistence-seam, and route-response helper exports | Existing route dependencies and authentication context where applicable; synthetic in-process request/response fixtures for harness tests; configured persistence/provider seams whose environment availability is not established here. | Implemented response contracts, deterministic route decisions, route exports, and synthetic harness helpers with explicit non-authorizations and fail-closed unavailable paths. | `APP_LOCAL_GOVERNED_ROUTE_AND_TEST_HARNESS_SEAMS / ENVIRONMENT_EFFECTS_UNOBSERVED` | Route/source and harness presence grants no persistence success, provider dispatch, execution, acceptance, customer, production, or activation authority. |
...
| `A9-CAP-010` | Passalong/thread-memory local boundary | App-local passalong records, queues, validation, route recommendation, copyable packet, and environment-dependent persistence adapter boundary. | [CAP010-TYPES]<br>[CAP010-SAMPLE]<br>[CAP010-ROUTER]<br>[CAP010-BOUNDARY]<br>[CAP010-PERSISTENCE] | `getThreadMemoryRecords`; `getPassalongRecords`; `buildInboxQueue`; `buildOutboxQueue`; `buildRouteRecommendation`; `buildCopyablePassalongPacket`; `buildPersistedPassalongInput`; `validatePersistedPassalongCreateInput`; `validatePersistedPassalongPatchInput`; `listPersistedPassalongRecords`; `persistPassalongRecord`; `updatePersistedPassalongRecord` | Import current modules; database availability is environment-dependent and not observed by this artifact. | Implemented local boundary and adapter with non-authoritative records, manual routing, validation, and unavailable fallbacks. | `APP_LOCAL_BOUNDARY / DURABLE_PERSISTENCE_UNOBSERVED` | No automatic send, route, execution, import, mutation authority, source-of-truth transfer, customer, provider, production, or activation effect. |
...
| `A9-CLM-002-IMP` | `A9-CAP-002` | `IMPLEMENTED` | `EVIDENCED` | Route-contract, decision, and route-export source | [CAP002-PASSALONG-CONTRACT], [CAP002-MOTION-CONTRACT], [CAP002-INFERENCE-CONTRACT], [CAP002-PASSALONG-DECISION], [CAP002-MOTION-DECISION], [CAP002-INFERENCE-DECISION], [CAP002-PASSALONG-COLLECTION-ROUTE], [CAP002-PASSALONG-DETAIL-ROUTE], [CAP002-MOTION-ROUTE], and [CAP002-INFERENCE-ROUTE] | `69ee1e0a059189c5014f048e991a4fc781a127b2` | `STATIC_CONFIGURATION` | Governed response contracts, deterministic decision seams, and route exports exist for the bounded routes. | `NONE_WITHIN_THIS_DIMENSION` | Implemented seams do not prove external effects. |
| `A9-CLM-002-BHT` | `A9-CAP-002` | `BEHAVIORALLY_TESTED` | `EVIDENCED` | Five focused bounded tests | [CAP002-ADOPTION-TEST], [CAP002-BOUNDARY-TEST], [CAP002-PARITY-TEST], [CAP002-DECISION-TEST], and [CAP002-HARNESS-TEST] | Required A9 validation | `STATIC_CONFIGURATION` | Adoption, static boundary, response parity, decision seam, and synthetic routeHarness boundaries have joined test coverage. | `NONE_WITHIN_THIS_DIMENSION` | Tests prove only named local contract, decision, route, and harness behavior. |
| `A9-CLM-002-LSH` | `A9-CAP-002` | `LOCAL_SHADOW` | `EVIDENCED` | App-local contracts and synthetic harness seams | [CAP002-PASSALONG-CONTRACT], [CAP002-MOTION-CONTRACT], [CAP002-INFERENCE-CONTRACT], [CAP002-HARNESS-FIXTURES], [CAP002-HARNESS-PROVIDER-SEAM], [CAP002-HARNESS-PERSISTENCE-SEAM], and [CAP002-HARNESS-ROUTE-RESPONSE] | `69ee1e0a059189c5014f048e991a4fc781a127b2` | `STATIC_CONFIGURATION` | Response contracts and synthetic fixtures, provider, persistence, and route-response harness helpers can be evaluated in process. | `NONE_WITHIN_THIS_DIMENSION` | Local harness evaluation does not establish provider dispatch, persistence, production, or local operating-loop handler behavior. |
...
The commands are exact. `local_result` records the completed A9 local validation
at `2026-07-22T23:32:18Z`, after the source/provenance cutoff; each pass proves
only the named bounded behavior and does not expand that cutoff.
...
| `A9-TST-006` | `A9-CAP-002` | [CAP002-HARNESS-TEST] | `corepack pnpm -C portal exec tsx src/lib/controlPlane/governedRouteHandlerBoundary.test.ts` | `PASS / exit 0 / 2026-07-22T23:32:18Z` | Synthetic request fixtures and provider, persistence, and route-response harness boundaries. | [CAP002-HARNESS-FIXTURES], [CAP002-HARNESS-PROVIDER-SEAM], [CAP002-HARNESS-PERSISTENCE-SEAM], and [CAP002-HARNESS-ROUTE-RESPONSE] | No production local operating-loop handler import, invocation, behavior, customer, persistence, or provider proof. |
...
| Immutable SHA/path resolution | `PASS / 53 pinned blob-reference definitions; 53 unique SHA/path blob targets; 14 pinned commit references resolved` |
...
[CAP002-HARNESS-FIXTURES]: https://github.com/jai-nexus/dev-jai-nexus/blob/69ee1e0a059189c5014f048e991a4fc781a127b2/portal/src/lib/controlPlane/routeHarness/fixtures.ts
[CAP002-HARNESS-PROVIDER-SEAM]: https://github.com/jai-nexus/dev-jai-nexus/blob/69ee1e0a059189c5014f048e991a4fc781a127b2/portal/src/lib/controlPlane/routeHarness/provider-seam.ts
[CAP002-HARNESS-PERSISTENCE-SEAM]: https://github.com/jai-nexus/dev-jai-nexus/blob/69ee1e0a059189c5014f048e991a4fc781a127b2/portal/src/lib/controlPlane/routeHarness/persistence-seam.ts
[CAP002-HARNESS-ROUTE-RESPONSE]: https://github.com/jai-nexus/dev-jai-nexus/blob/69ee1e0a059189c5014f048e991a4fc781a127b2/portal/src/lib/controlPlane/routeHarness/route-response.ts
...
[CAP002-HARNESS-TEST]: https://github.com/jai-nexus/dev-jai-nexus/blob/69ee1e0a059189c5014f048e991a4fc781a127b2/portal/src/lib/controlPlane/governedRouteHandlerBoundary.test.ts
