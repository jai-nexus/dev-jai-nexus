...
| Route | `CT-2026-07-22-Q3M7Y26-P1-START-A4-MOTION-DECISION-REGISTRY-v0` |
...
| Evidence cutoff | `2026-07-22T03:49:12Z` |
| GitHub query scope | Exact PRs `#376` through `#389`; all were returned merged |
| Open-PR collision query | Exact repository, open state; `0` results at cutoff |
...
| Current A4 route | Fresh HUMAN_OPERATOR/CONTROL_THREAD workflow instruction | Tier 1/2 workflow authority for this lane only; not an immutable repository source. |
...
6. `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` is used whenever the exact decision
   token is absent from immutable evidence. It is never reconstructed from a
   PR title, Linear state, or narrative memory.
...
| `source_authority_tier` | A2 tier and any mirror limitation. |
| `effective_scope_or_effect` | Bounded effect; never inferred authority. |
| `observed_status` | Exactly one A2 controlled status. |
| `currentness` | Current, historical phase-bound, workflow-only, or unavailable posture. |
...
| `evidence_class` | Workflow, repository, generated, founder, GitHub delivery, or mirror class. |
...
| `A4-M-002` | `MOTION_DELIVERY` | `motion-0248 / PR376` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR376], [C291] | `5 repository + 8 GitHub mirror` | Merges the nine-path package/snapshot delivery. | `ACCEPTED_CURRENT` | Merged event retained on current main. |
| `A4-M-003` | `MOTION_RECEIPT_REPAIR` | `motion-0248 / receipt coherence` | `CONTROL_THREAD route; Builder delivery` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [MOTION-EXECUTION], [PR377], [C3CC] | `2 workflow recorded by 5 repository; 8 PR mirror` | Corrects stale required-check wording without opening Program 1. | `ACCEPTED_CURRENT` | Current repaired receipt. |
...
| `A4-M-005` | `PROGRAM_OPENING_PACKET_DELIVERY` | `Q3M7Y26-P1 / opening readiness packet` | `Builder delivery; CONTROL_THREAD review required` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [OPENING-PACKET], [PR378], [C57F] | `5 repository + 8 PR mirror` | Records readiness for a later human decision; does not open Program 1. | `RATIFIED_PHASE_SPECIFIC` | Historical pre-opening packet. |
...
| `A4-A-003` | `SOURCE_HEAD_AND_DELIVERY` | `Q3M7Y26-P1:A1` | `Builder delivery + GitHub merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR380], [CCAC] | `5 repository + 8 PR mirror` | Delivers accepted-main reconciliation planning artifact. | `ACCEPTED_CURRENT` | Current accepted planning basis. |
| `A4-A-004` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:A1` | `CONTROL_THREAD` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [A2-ARTIFACT] | `2 accepted status recorded downstream by 5 repository` | A2 names A1 as accepted basis. | `ACCEPTED_CURRENT` | Exact acceptance token is absent; merge alone is not acceptance. |
...
| `A4-A-006` | `SOURCE_HEAD_AND_DELIVERY` | `Q3M7Y26-P1:A2` | `Builder delivery + GitHub merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR383], [CC7E] | `5 repository + 8 PR mirror` | Delivers the controlling A2 precedence/contradiction artifact. | `ACCEPTED_CURRENT` | Current controlling documentary source. |
| `A4-A-007` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:A2` | `CONTROL_THREAD` | `CT-2026-07-20-Q3M7Y26-P1-A2-ACCEPT-v0` | Current A4 route; no immutable receipt located. | `1 fresh human route + 2 workflow decision` | Accepts A2 documentary canon within Program 1. | `ACCEPTED_CURRENT` | Workflow-only token; current route explicitly supplies it. |
| `A4-A-008` | `LANE_ROUTE` | `Q3M7Y26-P1:A3` | `HUMAN_OPERATOR / CONTROL_THREAD` | `CT-2026-07-19-Q3M7Y26-P1-START-A2-A3-D3-v0` | Current A4 route and [PR383] route metadata. | `1/2 workflow; 8 PR corroboration` | Routes read-only A3 reconciliation evidence. | `RATIFIED_PHASE_SPECIFIC` | No durable A3 artifact is assumed. |
| `A4-A-009` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:A3` | `CONTROL_THREAD` | `CT-2026-07-20-Q3M7Y26-P1-A3-ACCEPT-v0` | Current A4 route; no immutable A3 artifact located. | `1 fresh human route + 2 workflow decision` | Marks the normalized A3 evidence packet accepted; creates no repository ownership. | `ACCEPTED_CURRENT` | Workflow-only; registry records the durable-evidence gap. |
| `A4-A-010` | `LANE_ROUTE` | `Q3M7Y26-P1:A4` | `HUMAN_OPERATOR / CONTROL_THREAD` | `CT-2026-07-22-Q3M7Y26-P1-START-A4-MOTION-DECISION-REGISTRY-v0` | Current A4 execution route. | `1 fresh human + 2 CONTROL_THREAD route` | Routes one-file documentary delivery and one Draft PR. | `ACCEPTED_CURRENT` | Current route; A4 acceptance is not included or implied. |
<!-- A4_BATCH_A_CORE_END -->
...
| registry_record_id | record_class | subject_coordinate | issuer | decision_token | source_ref | source_authority_tier | effective_scope_or_effect | observed_status | currentness |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `A4-EXC-001` | `BOUNDED_BATCH_EXCEPTION` | `Q3M7Y26-P1 / Batch D proving seam / D1-D8` | `HUMAN_OPERATOR / CONTROL_THREAD` | `AUTHORIZE_PROGRAM_1_BATCH_D_PROVING_SEAM_WITH_CI_FIRST` | Current A4 route records the exact token; no immutable receipt located. | `1 fresh human route + 2 workflow-only decision` | Permits bounded local/dev CI-first proving work out of the canonical Batch order through D8 only. | `RATIFIED_PHASE_SPECIFIC` | Terminated after accepted D8 head merged as `a1194b8...`. |
...
| registry_record_id | bounded_exception_prerequisites | termination_or_expiry_boundary | supersession_or_follow_up | evidence_class | evidence_ceiling | explicit_non_authorizations | freshness_or_explanatory_notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `A4-EXC-001` | Canonical prerequisites bypassed only for the proving seam: completed Batch A, accepted Batch B lifecycle canon, and accepted Batch C enforcement. | Exact D1-D8 proving sequence ended with D8 acceptance and squash `a1194b8...`; no continuing exception. | `phase-bounds` D1-D8; `does-not-supersede` canonical `Batch A -> B -> C -> D` dependencies. | Workflow-only bounded exception plus merged delivery evidence. | Tested/local-shadow and founder-observation evidence through D8 only. | No Batch D exit, D9, persistence, provider, Agent, Council, customer, production, Program exit, or JAI activation. | Exact token is not in immutable repository evidence; current route requires workflow-only classification. |
...
| `A4-D1-002` | `SOURCE_HEAD` | `Q3M7Y26-P1:D1` | `JAI::DEV::VERIFIER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D1-HEAD] | `5 repository source` | Adds bounded control-plane behavioral tests to CI. | `RATIFIED_PHASE_SPECIFIC` | Historical accepted source head; effect retained. |
| `A4-D1-003` | `DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D1` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR381], [CA0E] | `5 repository + 8 GitHub mirror` | Delivers D1 CI guardrail event. | `ACCEPTED_CURRENT` | Merged event on current main. |
...
| `A4-D2-002` | `SOURCE_HEAD` | `Q3M7Y26-P1:D2` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D2-HEAD] | `5 repository source` | Implements the tested local operating-loop proving seam. | `RATIFIED_PHASE_SPECIFIC` | Historical accepted source head; no production proof. |
| `A4-D2-003` | `DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D2` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR382], [C1B1] | `5 repository + 8 GitHub mirror` | Delivers D2 implementation and tests. | `ACCEPTED_CURRENT` | Merged event on current main. |
...
| `A4-D3-001` | `FOUNDER_OBSERVATION_ROUTE` | `Q3M7Y26-P1:D3` | `HUMAN_OPERATOR / CONTROL_THREAD` | `CT-2026-07-19-Q3M7Y26-P1-START-A2-A3-D3-v0` | Current A4 route and [PR383] route metadata. | `1/2 workflow; 8 PR corroboration` | Routes founder observation evidence only. | `RATIFIED_PHASE_SPECIFIC` | Historical workflow route. |
...
| `A4-D3-003` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:D3 / evidence packet` | `CONTROL_THREAD` | `CT-2026-07-20-Q3M7Y26-P1-D3-ACCEPT-v0` | Current A4 route; no immutable D3 artifact located. | `1 fresh route + 2 workflow decision` | Accepts founder-observation evidence only, with product disposition retained. | `ACCEPTED_CURRENT` | Distinct from founder disposition. |
...
| `A4-D4-002` | `SOURCE_HEAD` | `Q3M7Y26-P1:D4` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D4-HEAD] | `5 repository source` | Clarifies structural validation versus semantic deliberation. | `RATIFIED_PHASE_SPECIFIC` | Accepted source head distinct from squash. |
| `A4-D4-003` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:D4` | `CONTROL_THREAD` | `CT-2026-07-21-Q3M7Y26-P1-D4-ACCEPT-v0` | Current A4 route; no immutable decision receipt located. | `1 fresh route + 2 workflow decision` | Accepts bounded D4 evidence. | `ACCEPTED_CURRENT` | Workflow-only acceptance. |
| `A4-D4-004` | `DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D4` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR384], [C341] | `5 repository + 8 GitHub mirror` | Delivers accepted D4 source. | `ACCEPTED_CURRENT` | Source head `b07b...`; squash `3417...`. |
...
| `A4-D5-002` | `SOURCE_HEAD` | `Q3M7Y26-P1:D5 initial` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D5-INITIAL-HEAD] | `5 repository source` | Implements client recovery and invalidation baseline. | `RATIFIED_PHASE_SPECIFIC` | Historical initial source head. |
| `A4-D5-003` | `DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D5 initial` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR385], [CD18] | `5 repository + 8 GitHub mirror` | Delivers initial D5 implementation. | `ACCEPTED_CURRENT` | Later repair is a separate repository event. |
| `A4-D5-004` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D5R1` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D5R1-HEAD] | `5 repository source` | Adds fail-closed response coherence. | `RATIFIED_PHASE_SPECIFIC` | Repair token unavailable in immutable evidence. |
| `A4-D5-005` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D5R2` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D5R2-HEAD] | `5 repository source` | Binds transitions and deterministic evidence. | `RATIFIED_PHASE_SPECIFIC` | Repair token unavailable in immutable evidence. |
| `A4-D5-006` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D5R3` | `JAI::DEV::BUILDER` | `CT-2026-07-21-Q3M7Y26-P1-D5R3-TEST-EVIDENCE-CLOSURE-v0` | [D5R3-HEAD], [PR386] | `5 repository + 8 PR route metadata` | Adds coordinated false-evidence rejection tests. | `RATIFIED_PHASE_SPECIFIC` | Token is corroborated by PR metadata, which is not acceptance authority. |
...
| `A4-D5-009` | `FINAL_DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D5 final repair` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR386], [CF7F] | `5 repository + 8 GitHub mirror` | Delivers three-commit D5 repair sequence. | `ACCEPTED_CURRENT` | Final source head `e158...`; squash `f7f...`. |
| `A4-D6-001` | `LANE_ROUTE` | `Q3M7Y26-P1:D6` | `HUMAN_OPERATOR / CONTROL_THREAD` | `CT-2026-07-21-Q3M7Y26-P1-START-D6-EXPLAINABLE-RECOMMENDATION-v0` | [PR387] and current A4 route. | `1/2 workflow + 8 PR corroboration` | Routes explainable recommendation work. | `RATIFIED_PHASE_SPECIFIC` | Historical route. |
| `A4-D6-002` | `SOURCE_HEAD` | `Q3M7Y26-P1:D6 initial` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D6-INITIAL-HEAD] | `5 repository source` | Adds deterministic founder-readable explanations. | `RATIFIED_PHASE_SPECIFIC` | Initial source before repairs. |
| `A4-D6-003` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D6R1` | `JAI::DEV::BUILDER` | `CT-2026-07-21-Q3M7Y26-P1-D6R1-EXPLANATION-COHERENCE-v0` | [D6R1-HEAD], [PR387] | `2 route metadata + 5 repository + 8 PR` | Closes omitted-finding and pending-proof coherence gaps. | `RATIFIED_PHASE_SPECIFIC` | Historical repair. |
| `A4-D6-004` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D6R2` | `JAI::DEV::BUILDER` | `CT-2026-07-21-Q3M7Y26-P1-D6R2-DISPLAY-CONTAINMENT-v0` | [D6R2-HEAD], [PR387] | `2 route metadata + 5 repository + 8 PR` | Contains terminal display and staged drift; accepted source head. | `ACCEPTED_CURRENT` | Source head distinct from squash. |
| `A4-D6-005` | `FOUNDER_DISPOSITION` | `Q3M7Y26-P1:D6 / comprehension` | `HUMAN_OPERATOR` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR387] reports founder disposition `ACCEPT`; no immutable founder packet located. | `8 GitHub mirror reporting workflow evidence` | Records PR-reported founder comprehension only. | `MIRROR_ONLY` | Does not substitute for Control acceptance. |
| `A4-D6-006` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:D6` | `CONTROL_THREAD` | `CT-2026-07-21-Q3M7Y26-P1-D6-ACCEPT-v0` | Current A4 route; no immutable decision receipt located. | `1 fresh route + 2 workflow decision` | Accepts D6 bounded evidence. | `ACCEPTED_CURRENT` | Workflow-only acceptance. |
| `A4-D6-007` | `DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D6` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR387], [CE41] | `5 repository + 8 GitHub mirror` | Delivers the three-commit D6 sequence. | `ACCEPTED_CURRENT` | Source head `d980...`; squash `e41...`. |
| `A4-D7-001` | `LANE_ROUTE` | `Q3M7Y26-P1:D7` | `HUMAN_OPERATOR / CONTROL_THREAD` | `CT-2026-07-21-Q3M7Y26-P1-START-D7-DECISION-CONFIRMATION-v0` | [PR388] and current A4 route. | `1/2 workflow + 8 PR corroboration` | Routes decision confirmation and authority clarity. | `RATIFIED_PHASE_SPECIFIC` | Historical route. |
| `A4-D7-002` | `SOURCE_HEAD` | `Q3M7Y26-P1:D7 initial` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D7-INITIAL-HEAD] | `5 repository source` | Adds deliberate two-step decision confirmation. | `RATIFIED_PHASE_SPECIFIC` | Initial source before repair. |
| `A4-D7-003` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D7R1` | `JAI::DEV::BUILDER` | `CT-2026-07-21-Q3M7Y26-P1-D7R1-MALFORMED-CONFIRMATION-FAIL-CLOSED-v0` | [D7R1-HEAD], [PR388] | `2 route metadata + 5 repository + 8 PR` | Fails closed malformed confirmation inputs; accepted source head. | `ACCEPTED_CURRENT` | Source head distinct from squash. |
| `A4-D7-004` | `FOUNDER_OBSERVATION` | `Q3M7Y26-P1:D7` | `HUMAN_OPERATOR` | `CT-2026-07-21-Q3M7Y26-P1-D7-FOUNDER-OBSERVATION-v0` | Current A4 route; no immutable founder packet located. | `1 workflow event; 9 durable source unavailable` | Records that founder observation occurred; exact packet remains unavailable. | `UNAVAILABLE` | No browser finding is invented. |
| `A4-D7-005` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:D7` | `CONTROL_THREAD` | `CT-2026-07-21-Q3M7Y26-P1-D7-ACCEPT-v0` | Current A4 route; no immutable decision receipt located. | `1 fresh route + 2 workflow decision` | Accepts D7 bounded evidence. | `ACCEPTED_CURRENT` | Distinct from founder observation and merge. |
| `A4-D7-006` | `DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D7` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR388], [C8BB] | `5 repository + 8 GitHub mirror` | Delivers two-commit D7 sequence. | `ACCEPTED_CURRENT` | Source head `cd689...`; squash `8bb...`. |
| `A4-D8-001` | `LANE_ROUTE` | `Q3M7Y26-P1:D8` | `HUMAN_OPERATOR / CONTROL_THREAD` | `CT-2026-07-22-Q3M7Y26-P1-START-D8-WORK-PACKET-EXPORT-v0` | [PR389] and current A4 route. | `1/2 workflow + 8 PR corroboration` | Routes redacted boundary-receipt export work. | `RATIFIED_PHASE_SPECIFIC` | Historical route. |
| `A4-D8-002` | `SOURCE_HEAD` | `Q3M7Y26-P1:D8 initial` | `JAI::DEV::BUILDER` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [D8-INITIAL-HEAD] | `5 repository source` | Adds deterministic redacted receipt and clipboard control. | `RATIFIED_PHASE_SPECIFIC` | Initial source before repairs. |
| `A4-D8-003` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D8R1` | `JAI::DEV::BUILDER` | `CT-2026-07-22-Q3M7Y26-P1-D8R1-DESCRIPTOR-SNAPSHOT-FAIL-CLOSED-v0` | [D8R1-HEAD], current A4 route. | `2 workflow + 5 repository` | Snapshots descriptor inputs before validation. | `RATIFIED_PHASE_SPECIFIC` | Intermediate repair head. |
| `A4-D8-004` | `FOUNDER_DISPOSITION` | `Q3M7Y26-P1:D8 initial observation` | `HUMAN_OPERATOR` | `CT-2026-07-22-Q3M7Y26-P1-D8-FOUNDER-OBSERVATION-v0` | [PR389] reports `REVISE` for focus loss; current A4 route supplies token. | `1 workflow event + 8 PR mirror` | Requires clipboard-focus correction only. | `RATIFIED_PHASE_SPECIFIC` | Phase-bound to head `8582dd8...`; not acceptance. |
| `A4-D8-005` | `REPAIR_SOURCE_HEAD` | `Q3M7Y26-P1:D8R2` | `JAI::DEV::BUILDER` | `CT-2026-07-22-Q3M7Y26-P1-D8R2-CLIPBOARD-FOCUS-RETENTION-v0` | [D8R2-HEAD], current A4 route. | `2 workflow + 5 repository` | Preserves natural clipboard-button focus and single-flight behavior; accepted source head. | `ACCEPTED_CURRENT` | Source head distinct from squash. |
| `A4-D8-006` | `FOUNDER_REOBSERVATION` | `Q3M7Y26-P1:D8R2` | `HUMAN_OPERATOR` | `CT-2026-07-22-Q3M7Y26-P1-D8R2-FOUNDER-REOBSERVATION-v0` | Current A4 route; immutable founder packet not located. | `1 workflow event; 9 durable source unavailable` | Records re-observation occurrence; exact browser packet is unavailable. | `UNAVAILABLE` | Later Control acceptance is separately recorded. |
| `A4-D8-007` | `CONTROL_ACCEPTANCE` | `Q3M7Y26-P1:D8` | `CONTROL_THREAD` | `CT-2026-07-22-Q3M7Y26-P1-D8-ACCEPT-v0` | Current A4 route; no immutable decision receipt located. | `1 fresh route + 2 workflow decision` | Accepts D8 bounded evidence at source head `4b29156...`. | `ACCEPTED_CURRENT` | Does not prove external-effect absence. |
| `A4-D8-008` | `DELIVERY_AND_SQUASH` | `Q3M7Y26-P1:D8` | `GitHub delivery + repository merge` | `UNAVAILABLE_NOT_IN_IMMUTABLE_SOURCE` | [PR389], [CA11] | `5 repository + 8 GitHub mirror` | Delivers three-commit D8 sequence onto current main. | `ACCEPTED_CURRENT` | Source head `4b29156...`; squash/current main `a1194b8...`. |
<!-- A4_BATCH_D_CORE_END -->
...
| registry_record_id | bounded_exception_prerequisites | termination_or_expiry_boundary | supersession_or_follow_up | evidence_class | evidence_ceiling | explicit_non_authorizations | freshness_or_explanatory_notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `A4-D1-001` | Batch A/B/C canonical prerequisites bypassed only by `A4-EXC-001`. | Ends at D1 delivery. | Required follow-up was D1 source/delivery evidence; acceptance token remains unavailable. | Workflow route unavailable. | CI routing only. | Common non-authorizations; no acceptance by route. | Exact D1 token must not be reconstructed. |
...
| `A4-D2-001` | Batch A/B/C bypassed only by `A4-EXC-001`; D1 CI prerequisite delivered. | Ends at D2 delivery. | `records-later-event` to `A4-D2-003`. | Workflow route unavailable. | Local-shadow route only. | Common non-authorizations; no production route. | Exact D2 token must not be reconstructed. |
...
| `A4-D6-001` | D5 repository basis and exception. | Ends at D6 acceptance/hold. | `records-later-event` to D6 source/repairs. | Repository-recorded workflow route. | Explainability implementation only. | Common non-authorizations; no server recommendation transfer. | Exact route in PR body. |
...
| `A4-D6-003` | D6 initial source and coherence findings. | Ends at D6R2. | `records-later-event` to `A4-D6-004`. | Repository repair source. | Explanation/proof coherence only. | Common non-authorizations; no client-derived recommendation display. | Commit `91300...` verified. |
| `A4-D6-004` | D6R1 and founder findings. | Source phase ends at squash; accepted evidence remains. | `records-later-event` to Control acceptance and delivery. | Repository repair source. | Tested deterministic contracts plus bounded founder evidence. | Common non-authorizations; no external-effect proof. | Final source head `d980...`. |
| `A4-D6-005` | Authenticated founder re-observation. | Phase-bound to D6 acceptance. | `does-not-supersede` Control acceptance; corroborates comprehension only. | GitHub PR mirror of founder evidence. | Founder comprehension report only. | Common non-authorizations; no authority from founder UX result. | Exact founder token unavailable. |
| `A4-D6-006` | D6 source, repair, and founder evidence reviewed. | Until corrected/superseded. | `records-later-event` before merge; distinct from `A4-D6-007`. | Workflow-only Control acceptance. | Accepted D6 bounded evidence. | Common non-authorizations; no D7 route by implication. | Exact acceptance token supplied by current route. |
| `A4-D6-007` | Accepted source head and merge authorization. | Delivery complete at squash. | `does-not-supersede` Control acceptance; records delivery. | Merged repository event plus PR metadata. | D6 bounded evidence only. | Common non-authorizations; no authority by merge. | Merged `2026-07-21T21:57:45Z`. |
...
| --- | --- | --- | --- | --- |
| `A4-EXC-001` | Completion/acceptance of the remaining Batch A sequence plus canonical Batch B lifecycle and Batch C enforcement prerequisites. | CI-first local/dev proving seam, D1-D8 only. | D8 accepted source head `4b29156...` merged as `a1194b8...`. | `CLOSED_PHASE_BOUND`; grants no continuing Batch D, D9, or other authority. |
...
| Canonical Batch order | `phase-bounds` | `A4-EXC-001` | Exception permits D1-D8 proving only; it does not replace Batch dependencies. |
...
| D6 initial source | `records-later-event` | D6R1 and D6R2 | Repair heads preserve provenance and culminate in accepted source head. |
...
| Batch A | A1-A3 complete per fresh route; A4 routed but not accepted. | `A4-A-004`, `A4-A-007`, `A4-A-009`, `A4-A-010` | No Batch A exit. |
...
| Durable D6 founder packet/token | `UNAVAILABLE` | PR-reported `ACCEPT` remains `MIRROR_ONLY`; Control acceptance is separate. |
...
| Immutable A4 route receipt | `UNAVAILABLE` | Current fresh route governs this lane; the registry cannot accept itself. |
...
| Audit | Result |
| --- | --- |
| Registry ID uniqueness | `PASS - 64 unique core IDs and no duplicates.` |
| Core/boundary one-to-one join | `PASS - 64 core rows and 64 boundary rows; zero join mismatches.` |
...
| Exact known Control-token scan | `PASS - all 21 required Control and bounded-exception tokens are present.` |
| Unavailable-token handling | D1, D2, and D5 acceptance tokens remain explicitly unavailable. |
...
- Open PR collision query: none.
- Authorized path before authoring: absent.
- GitHub PR metadata: exact PRs #376-#389 returned merged.
...
| Required headings and fields | `PASS - 19 numbered sections; 64 complete 17-field joined records.` |
| Unique IDs and paired-table join | `PASS - 64/64 unique core/boundary rows with zero mismatches.` |
| Immutable path/commit pointers | `PASS - all 29 commit and 7 pinned path references resolve.` |
| A2 vocabulary and relation audit | `PASS - controlled statuses and permitted relation semantics only.` |
| Exception/currentness audit | `PASS - one closed phase-bound exception; no continuing D9 or Batch authority.` |
...
| All 15 focused behavioral tests | `PASS - 15/15 exited 0; exact unchanged commands used the authorized outside-sandbox rerun after the established tsx listen EPERM restriction.` |
...
Rollback is a normal revert of the single documentary commit. Revert removes
the index only; it does not reverse, reject, or alter any indexed Motion,
Program, route, observation, acceptance, source commit, merge, or mirror.
...
`NEXT_REQUIRED_DECISION: ACCEPT_A4_BUILDER_PR_FOR_INDEPENDENT_VERIFICATION`
