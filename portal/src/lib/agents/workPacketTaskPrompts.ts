...
export function buildDraftWorkPacketTaskPrompt(
  ...
  const promptLines = [
    ...
    `Agenda status: ${packet.status}`,
    `Source kind: ${packet.source.kind}`,
    `Source label: ${packet.source.label}`,
    `Source motion: ${packet.source.motion_id ?? "none"}`,
    `Assigned identity (${packet.agent.agent_class === "palette_draft" ? "palette draft" : "canonical active"}): ${packet.agent.key}`,
    `Assigned agent display name: ${packet.agent.label}`,
    `Assigned agent handle: ${packet.agent.handle}`,
    `Canonical role: ${packet.canonical_role.canonical_role_key ?? "none"}`,
    ...
    `- target: ${packet.next_prompt_target.target}`,
    `- label: ${packet.next_prompt_target.label}`,
    `- prompt: ${packet.next_prompt_target.prompt}`,
    ``,
    `Allowed paths:`,
  ...
  return {
    ...
    canonical_role_key: packet.canonical_role.canonical_role_key,
    canonical_role_label: packet.canonical_role.canonical_role_label,
    palette_draft_key: packet.canonical_role.palette_draft_key,
    palette_draft_label: packet.canonical_role.palette_draft_label,
    next_prompt_target: packet.next_prompt_target.target,
    next_prompt_label: packet.next_prompt_target.label,
    next_prompt_text: packet.next_prompt_target.prompt,
