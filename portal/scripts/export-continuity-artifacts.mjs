...
function renderWaveYaml(wave) {
  const wavePayload = {
    ...
    planning_sections: wave.planning_sections,
...
function renderWavePlanMarkdown(wave) {
  const lines = [
    ...
    "",
    ...wave.planning_sections.current_repo_framing.map((item) => `- ${item}`),
    "",
    "## Problem Statement",
    "",
    ...wave.planning_sections.problem_statement.map((item) => `- ${item}`),
    "",
    "## Customer Portal Intake Thesis",
    "",
    ...wave.planning_sections.intake_thesis.map((item) => `- ${item}`),
    "",
    "## Billing / Stripe Assumptions And Deferrals",
    "",
    ...wave.planning_sections.billing_assumptions_and_deferrals.map((item) => `- ${item}`),
    "",
    "## Auth / Account Assumptions And Deferrals",
    "",
    ...wave.planning_sections.auth_assumptions_and_deferrals.map((item) => `- ${item}`),
    "",
    "## Workspace / Project Planning",
    "",
    ...wave.planning_sections.workspace_project_planning.map((item) => `- ${item}`),
    "",
    "## Evidence And Gates",
    "",
    ...wave.planning_sections.evidence_and_gates.map((item) => `- ${item}`),
    "",
    "## Deferred Execution Authority",
    "",
    ...wave.planning_sections.deferred_execution_authority.map((item) => `- ${item}`),
    "",
    "## Repo-facing Passalong Prompt",
    "",
    "```text",
    wave.planning_sections.repo_passalong_prompt,
    "```",
    "",
    "## Next Prompt Preview",
    "",
