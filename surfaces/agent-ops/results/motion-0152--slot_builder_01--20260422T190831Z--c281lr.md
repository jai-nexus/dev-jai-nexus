...
Document only TaskContract and ResolvedTaskContract fields, defaults/constants present in runtime/taskContract.ts, and exact resolveTaskContract behavior. Exclude examples, status labels, operator workflow, and manual-application guidance. Output structured markdown for .nexus/docs/task-contract-v0.md.
...
3. allowed_scope
   ...
   - Else:
     ...
     - Apply normalizeStringList to the resulting array (this preserves [] when objective is empty).
